...
export function App({ api }: { api: KeyflareApi }) {
  ...
  async function buildFirmware() {
    ...
    try {
      ...
      for (const id of channels) {
        ...
        if (kind === "rgb-indicator" && effectiveIndicatorLeds[id] === undefined) {
  ...
  function toggleChannel(channel: ChannelId) {
    const kind = capabilities?.channels.find((entry) => entry.id === channel)
      ?.kind;
...
function KeyboardPreview({
  ...
  const primaryKeys = layout.keys.filter((key) => {
    ...
    const isAlternate = drawnForBounds.some(
      (r) => x < r.x1 - 0.01 && r.x0 < x + w - 0.01 && y < r.y1 - 0.01 && r.y0 < y + h - 0.01,
    );
                ...
                {rgbLeds.map((led, index) => {
                  ...
                  return (
                    <g key={`led-${index}`} role="listitem">
                      <circle
                        className={isChosen ? "led-dot led-dot-chosen" : "led-dot"}
                        cx={cx}
                        cy={cy}
                        r={9}
                        fill={fill}
                        onClick={() => onSelectIndicatorLed?.(index)}
                        style={onSelectIndicatorLed ? { cursor: "pointer" } : undefined}
                      >
